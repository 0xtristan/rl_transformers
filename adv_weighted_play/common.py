...
def data_to_r_buf_episodes(data, env):
    n_trajs = len(data['obs'])
    episodes = []

    for e in range(0, n_trajs):
        ep = []
        for t in range(0, len(data['obs'][e]) - 1):  # -1 because we want o and o2.

            o = {'observation': data['obs'][e][t], 'achieved_goal': data['achieved_goals'][e][t],
                 'desired_goal': data['desired_goals'][e][t],
                 'controllable_achieved_goal': data['controllable_achieved_goals'][e][t],
                 'full_positional_state': data['full_positional_states'][e][t]}
            o2 = {'observation': data['obs'][e][t + 1], 'achieved_goal': data['achieved_goals'][e][t + 1],
                  'desired_goal': data['desired_goals'][e][t + 1],
                  'controllable_achieved_goal': data['controllable_achieved_goals'][e][t + 1],
                  'full_positional_state': data['full_positional_states'][e][t + 1]}
            a = data['acts'][e][t]
            r = env.compute_reward(o2['achieved_goal'], o2['desired_goal'])
            d = 0

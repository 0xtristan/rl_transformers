...
class SAC_model():
    ...
    def compute_loss_q(self,data):
        ...
        q_random = torch.log(torch.exp( (self.ac.q1(o, a_random)  / a_random.norm(dim=-1)).mean() ) + torch.exp( (self.ac.q2(o, a_random)  / a_random.norm(dim=-1)).mean()))
    ...
    def compute_loss_pi(self,data):
        o = data['obs']
        pi, logp_pi, distrib = self.ac.pi(o)

        if self.AWAC:
            a = data['act']
            logprob = distrib.log_prob(a).sum(axis=-1)
            with torch.no_grad():
                adv_weighting = torch.min( self.ac.q1(o, a), self.ac.q2(o,a) )
                Lambda = 0.3

            loss = -logprob
            loss = loss * torch.exp((1/Lambda) * adv_weighting)
            loss = loss.mean()
            ...
            return loss_pi, pi_info, q_pi.mean()
    ...
    def update(self,data):
        ...
        loss_pi, pi_info, q_pi = self.compute_loss_pi(data)
        loss_pi.backward()
